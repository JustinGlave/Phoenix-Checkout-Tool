...
checkout_tool_backend.py  — Data and storage logic (Job / ValveCheckout / CheckoutStore)
...
### Job
A job container that holds one or more `ValveCheckout` records.
- `id` — unique UUID
- `job_number` — ATS job number string
- `job_name` — project/job name string

### ValveCheckout
One complete checkout record mirroring the Phoenix Celeris checkout sheet.
- `id` — unique UUID
...
- `config` — dict of `{key}_cfm` and `{key}_notes` for CONFIG_ROWS
- `verification` — dict of `{key}_result` and `{key}_notes` for VERIFY_ROWS
- `notes` — free-form notes string

Data stored at `%APPDATA%\ATS Inc\Phoenix Valve Checkout Tool\data.json`.
...
└── Central QWidget (QHBoxLayout)
    ├── Left sidebar (QTreeWidget, fixed 270px width)
    │   ├── "+ New Job" and "+ New Checkout" buttons at top
    │   └── Tree: Job nodes → ValveCheckout children (sorted by valve_tag)
    └── Right main area (QWidget, stretch)
        ├── Header panel (valve tag title, subtitle, Pass/Fail badge)
        ├── Tab widget:
        │   ├── General tab (all header fields)
        │   ├── Wiring tab (Phoenix + Black Box wiring tables side-by-side)
        │   ├── Config & Verification tab (two tables)
        │   └── Notes tab (plain text)
